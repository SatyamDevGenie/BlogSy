...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const { data } = await axios.get("https://blogsy-571e.onrender.com/api/users/profile", config);
  ...
  const handleRemoveFromFavourites = async (blogId) => {
    try {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      await axios.delete(`https://blogsy-571e.onrender.com/api/users/favourites/${blogId}`, config);
      fetchProfile();
    } catch (err) {
